...
async def test_failure_before_the_ask_still_degrades(hass, jev, calls) -> None:
    entry, controller = await setup(hass)
    with patch(
        "custom_components.jev_autopilot.controller.build_plan",
        side_effect=ValueError("Score accepts at most 10 levels"),
    ):
        for _ in range(3):
            await controller.async_run()
        await hass.async_block_till_done()
