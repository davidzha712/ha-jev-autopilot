...
- **干净交还。** 你为房间列出的现有自动化，在自动驾驶接管期间关闭；在你关掉自动驾驶、
  Jev 连续 3 次不可达或回复无法执行、当日预算用完、API key 被拒、或 Home Assistant
  关机时重新打开。Home Assistant 启动完成后再次接管。
- **隐私。** 发给 Jev 的文本包含设备名、状态和传感器数值，住户只以人数出现（"3 人中 2 人在家"）。
  ...
  *先问我* 的通知发到你自己的手机，不发给 Jev，里面按你写的原样带房间名和设备名。
  手机 App 通过 Apple 或 Google 的推送服务投递这些通知。
...
| 亮度档位 (%) | `10, 30, 50, 75, 100` | 2 到 10 档，Jev 在这个刻度上选点 |
